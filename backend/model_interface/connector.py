...
class ModelDebugger:
    ...
    def get_improvement_suggestions(self, detail_level="comprehensive"):
        ...
        if not HAS_IMPROVEMENT_ADVISOR:
            ...
            return self.generate_improvement_suggestions()  # Use the existing method as fallback
    ...
    def generate_improvement_suggestions(self, detail_level="comprehensive") -> Dict[str, Any]:
        ...
        if hasattr(self, 'ground_truth') and self.ground_truth is not None and len(self.ground_truth) > 0:
            try:
        ...
        if hasattr(self, '_feature_importances') and self._feature_importances is not None:
        ...
        if detail_level == "code" and HAS_CODE_GENERATOR:
            try:
                ...
                for suggestion in suggestions:
                    ...
                    if "class imbalance" in suggestion["title"].lower() and hasattr(self, 'ground_truth') and self.ground_truth is not None:
                        try:
                            class_counts = np.bincount(self.ground_truth)
                            model_context["class_distribution"] = {
                                int(i): int(count) for i, count in enumerate(class_counts)
                            }
                        except Exception as e:
                            print(f"Error adding class distribution to context: {str(e)}")
                    elif "overfitting" in suggestion["issue"].lower() and hasattr(self, 'training_history'):
